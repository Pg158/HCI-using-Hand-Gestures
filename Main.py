    ...
    if mode == 'Brightness':
        ...
        if len(lmList) != 0:
            if fingers[-1] == 1:
                ...
                length = math.hypot(x2 - x1, y2 - y1)
                brightness = np.interp(length, [hmin, hmax], [0, 100])
                sbc.set_brightness(int(brightness))
                cv2.rectangle(img, (580, 150), (605, 400), (0, 215, 255), 3)
                bBar = np.interp(brightness, [0, 100], [400, 150])
                cv2.rectangle(img, (580, int(bBar)), (605, 400), (255, 255, 127), cv2.FILLED)
                cv2.putText(img, f'{int(brightness)}%', (560, 430), cv2.FONT_HERSHEY_COMPLEX, 0.9, (0, 215, 255), 3)
    ...
    if mode == 'Cursor':
        ...
        if fingers[1:] == [0, 0, 0, 0]:
            ...
            if len(lmList) != 0:
                ...
                # Limiting the cursor position to the screen boundaries
                X = int(np.interp(x1, [110, 620], [0, w - 1]))
                Y = int(np.interp(y1, [20, 350], [0, h - 1]))

                # Apply boundaries to keep the cursor within screen limits
                X = np.clip(X, 0, w - 1)
                Y = np.clip(Y, 0, h - 1)
                ...
                # Clicking action
                if fingers[0] == 0:
                    # Avoid clicking if cursor is near the edge of the screen
